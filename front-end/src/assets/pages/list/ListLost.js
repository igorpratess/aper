...
class ListLost extends React.Component {
    ...
    async getItems() {
        const response = await api.get('/listing/lost', {})
            .then(res => {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            let li = <li>
                <div className="div-items">
                    <h3 className="title">Perdido</h3>
                    <span>Local onde foi perdido: {list[i].location}</span>
                    <span>Tipo de item: {list[i].typeItem}</span>
                    <span>Nome do item: {list[i].name}</span>
                    <span>Data que foi perdido: {date}</span>
                    {description}
